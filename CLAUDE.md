...
- **Stage 2 complete (2026-04-18).**
  ...
  - `scoring/diff.py` runs end-to-end against a synthetic Medivation extraction (generated by `scripts/synth_extraction.py`) and emits a human-review markdown + JSON report with verdict checkboxes per divergence. Commit `0b0d4d7`.
...
- **Stage 3 gate.** Now open. Do not run on the 392 target deals until all 9 reference deals are manually verified against their filings AND the rulebook is stable across 3 consecutive unchanged-rulebook runs. Next actions — pick up in a fresh chat:
  1. Implement `run.py`'s `run_pipeline()` (Extractor → Validator via the Claude Agent SDK) using `prompts/extract.md` + `prompts/validate.md` + `rules/*.md`.
  2. Run on Medivation (simplest archetype). Diff against `reference/alex/medivation.json`. Austin reads the filing at `data/filings/medivation/raw.md` and adjudicates every divergence.
  3. Roll out per CLAUDE.md order: Medivation → Imprivata → Zep → Providence → Penford → Mac Gray → Petsmart → STec → Saks.
  4. Iterate rulebook + prompts until 3 consecutive full-reference-set runs produce the same output with no rule changes. Only then turn the crank on target deals.
