...
5. **Classify events.** Use the closed vocabulary in `rules/events.md` §C1. If an event doesn't fit, flag the ambiguity rather than invent a label. Key cross-references:
   ...
   **Drop classification doctrine (§I1 / §P-D7).** The filing's verb subject controls `drop_initiator`: target/board/committee/management/advisor rejects, excludes, does not advance, or declines to continue → `target`; bidder withdraws, declines, stops, fails to respond, or fails to submit → `bidder`; use `unknown` only when agency is genuinely ambiguous and flag it. Specific reason classes beat generic ones: target non-advancement / not invited / not selected → `"never_advanced"`; target threshold, reserve, below-minimum, below superior offer, or refusal to match → `"below_minimum"`; target financing-risk / certainty / antitrust / diligence / board-concern rationale → `"target_other"`; bidder no-response / failure to submit or reiterate after a request → `"no_response"`; asset/transaction-scope mismatch → `"scope_mismatch"` with initiator from the verb subject. Imprivata strategic bidders who were "no longer interested" are `bidder` + `null`; Mac Gray Party C's failure to submit or reiterate is `bidder` + `"no_response"`; target cuts for below a stated threshold or refusal to match a superior offer are `target` + `"below_minimum"`; target cuts for failure to advance past the informal screen are `target` + `"never_advanced"`.

   **Final-round fixtures to mirror.** Process letters requesting final bids are `Final Round` with `final_round_announcement = true`; the later deadline/submission event is another `Final Round` with `final_round_announcement = false`. Requests for revised written preliminary proposals are `final_round_informal = true`; binding / best-and-final / merger-agreement-draft requests are `final_round_informal = false`; later best-and-final requests after a prior deadline set `final_round_extension = true`.
