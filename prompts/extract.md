...
- [ ] **NDA-before-bid.** Every bidder with a `Bid` row has a preceding `NDA` row in the same phase (§P-D6 precondition). If a later event names a bidder who has no earlier NDA row, go back and emit the NDA row (even if only cited via a count-enumeration passage). **Exempt rows** carry either `unsolicited_first_contact` (§D1.a) or `pre_nda_informal_bid` (§C4) — the validator skips these; do NOT fabricate an NDA row to satisfy §P-D6 when the filing narrates a genuine NDA-less Bid.
