...
The Python validator will hard-flag structural violations (source-quote presence, vocabulary, §P-D2 rough-date symmetry, §P-D3 BidderID ordering, §P-D6 NDA-before-bid, §P-G2 bid_type evidence (range OR ≤300-char note), §P-S4 Executed-row presence and phase, etc.) and force a retry. Before returning the JSON, audit the **judgment calls** the validator can't see:
...
- [ ] **Executed atomization.** Consortium winners get one `Executed` row per merger-agreement signer named in the signature block. Do not collapse consortium signers into one row.
