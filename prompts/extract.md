...
5. **Classify events.** Use the closed vocabulary in `rules/events.md` §C1. If an event doesn't fit any vocabulary item, stop — flag the ambiguity rather than invent a new label. Per `rules/events.md` §D1, when an unsolicited bid is itself the first contact, emit the `Bid` row only and note that it initiated the process; do **not** duplicate it with a standalone `Bidder Sale` row. **Per `rules/events.md` §D1.a (iter-4), when that unsolicited-first-contact bidder NEVER signs an NDA in the deal** (target declines, bidder withdraws), attach `{"code": "unsolicited_first_contact", "severity": "info", "reason": "<short summary; MUST include a verbatim verb from {declined, did not respond, withdrew, rejected, terminated, ceased} with a quoted source snippet>"}` to the Bid row to exempt it from §P-D6's NDA-before-Bid check. **Per `rules/events.md` §D1.b (iter-4), multi-activist narratives emit one `Activist Sale` row per separately-narrated activist**; collapse only when the filing describes a coordinated group. Per `rules/events.md` §K2, when a final-round invitation date and a later process-letter date differ, anchor the `... Ann` row on the earlier invitation / advancement date. **Per `rules/bids.md` §C4 (iter-4; iter-5 clarification), concrete pre-NDA price indications emit a `Bid` row with `bid_type="informal"` plus `{"code": "pre_nda_informal_bid", "severity": "info", "reason": "<summary>"}` to document the pre-NDA timing** — do NOT use `Bidder Sale` or a `pre_nda_bidder_sale` flag (the saks-extractor ad-hoc convention is deprecated). §C4 does NOT need a §P-D6 exemption: the bidder's later NDA (same phase) satisfies the existence-only check.
