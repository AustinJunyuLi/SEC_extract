...
8. **Classify bids.** Apply `rules/bids.md` §G1 to decide `bid_type ∈ {"informal", "formal"}`. Every bid row emits `bid_note = "Bid"` per `rules/events.md` §C3 (the unified-bid convention); `bid_type` is the ONLY distinguisher between informal and formal. Do NOT emit non-vocabulary bid-row labels — they fail validator check §P-R3. Every non-null `bid_type` SHOULD carry a `bid_type_inference_note: str` (non-empty, ≤300 chars) justifying the classification unless the row is a true range bid or the classification is supplied by the paired/fallback `Final Round.final_round_informal` value. Range bids lock `bid_type = "informal"` — never emit a range-shaped Bid row with `bid_type = "formal"`. If a formal trigger phrase appears alongside a range, attach the soft flag `range_with_formal_trigger_override` (range still wins). Validator §P-G2 enforces range, note, or final-round fallback evidence.
   ...
   **Bid consideration completeness (§H2 / §P-R9).** Never emit a value-bearing `Bid` with `consideration_components: null`. For every `Bid` row with `bid_value`, `bid_value_pershare`, `bid_value_lower`, or `bid_value_upper`, set both `bid_value_unit` and a non-empty `consideration_components` list. Dollar-denominated per-share acquisition proposals are `["cash"]` unless the bid sentence, surrounding transaction description, or deal context states or implies stock, CVR, earnout, mixed consideration, or an implied securities value. Preserve non-cash or mixed terms when supported; do not use the final signed deal's all-cash status to overwrite a competing bid's stated structure.
...
- [ ] **Kept §R1 fields.** Deal-level `all_cash`, `target_legal_counsel`, `acquirer_legal_counsel`, and `deal_flags` are present; every value-bearing `Bid` row has non-empty `consideration_components`, every no-value `Bid` row has `consideration_components = null`, every non-Bid row has `consideration_components = null`, and each row has `exclusivity_days` populated or explicitly null.
