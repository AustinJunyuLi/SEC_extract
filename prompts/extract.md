...
5. **Classify events.** Use the closed vocabulary in `rules/events.md` §C1. If an event doesn't fit, flag the ambiguity rather than invent a label. Key cross-references:
   ...
   **Final-round fixtures to mirror.** Process letters requesting final bids are `Final Round` with `final_round_announcement = true`; the later deadline/submission event is another `Final Round` with `final_round_announcement = false`. A `Bid` row does not replace that non-announcement `Final Round` row; emit both when the filing supports both the deadline/submission milestone and the bidder's bid. Requests for revised written preliminary proposals are `final_round_informal = true`; binding / best-and-final / merger-agreement-draft requests are `final_round_informal = false`; later best-and-final requests after a prior deadline set `final_round_extension = true`.
