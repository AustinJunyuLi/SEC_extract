...
9. **Apply skip rules.** Do NOT emit rows for events that match `rules/bids.md` §M1 (unsolicited, no NDA, no price) or §M3 (legal advisor NDA). §M2 is folded into §I1's NDA-only rule (no skip; emit `DropSilent` per Step 7); §M4 is an emit-two-rows cross-phase NDA continuity rule, not a skip (see self-check).
