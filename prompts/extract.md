...
7. **Handle group/joint/aggregate rows.** Apply `rules/bidders.md` §E1 (atomize events), §E2 (joint bidders), §E2.a (Executed collapse to one row), §E2.b (NDA granularity follows filing narration), §E3 (placeholder counts: exact counts stay exact; `"several"` = 3 minimum; vaguer plurals emit one placeholder plus ambiguity flag). See `rules/bidders.md` for the full decision tables and `joint_bidder_members` / `joint_nda_aggregated` flag shapes.
