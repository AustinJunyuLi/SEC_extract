...
- **`bidder_type.public` is strict-filing-only and tri-state (§F1/§F2).** Set `public = true` only when the filing affirmatively states publicly traded (*"a publicly traded pharmaceutical company"*, *"NYSE-listed"*, *"NASDAQ-listed"*, *"public company"*). Set `public = false` only when the filing affirmatively states non-public (*"a private company"*, *"a privately held [industry] firm"*). Otherwise emit `null` — including for named PE sponsors whose firm-level listing the filing does not address (KKR, Blackstone, Apollo etc. are themselves listed sponsor firms; the *"private equity firm"* descriptor describes the fund vehicle, not the parent firm). Do **not** infer `public = true` from name recognition of strategic acquirers; if the filing did not say "publicly traded," emit `null`. PE-sponsor rows (`base = "f"`) get `public` independently per the same rule — there is no automatic `public = false` for PE sponsors.
...
- **`Acquirer` is the operating acquirer, not the legal shell (§N4).** Per `rules/schema.md` §N4, populate `Acquirer` with the entity that actually negotiated and will own the target's assets. Skip Delaware shells / merger-vehicle entities formed solely to execute the transaction (typically named `<Word> Holdings Inc.`, `<Word> Acquisition Inc.`, `<Word> Parent Inc.`, `<Word> Merger Sub` etc.) — these go in `Acquirer_legal` instead. **For PE consortia / club deals**, populate `Acquirer` with the **lead sponsor** the filing identifies in the primary position ("BC Partners, together with …"); fall back to the filing's verbatim consortium label (e.g., `"Buyer Group"`) only when no lead is identifiable. **For sponsor-backed corporate buyers** (operating company funded by a sponsor that is not itself the bidder, e.g., CSC ServiceWorks funded by Pamplona for mac-gray), use the operating company in `Acquirer` and document the funding sponsor in the `Executed` row's `additional_note`. Set `Acquirer_legal = null` when the legal entity and operating acquirer are the same single firm; otherwise put the shell name there.
