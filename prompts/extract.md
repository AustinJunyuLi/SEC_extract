...
5. **Classify events.** Use the closed vocabulary in `rules/events.md` §C1. If an event doesn't fit, flag the ambiguity rather than invent a label. Key cross-references:
   ...
   **Drop fixtures to mirror.** Imprivata strategic bidders who were "no longer interested" are `drop_initiator = "bidder"`, `drop_reason_class = null`. Mac Gray Party C's failure to submit or reiterate is `bidder` + `"no_response"`. Target cuts for below a stated threshold or refusal to match a superior offer are `target` + `"below_minimum"`. Target cuts for failure to advance past the informal screen are `target` + `"never_advanced"`. Financing-risk or board-concern target cuts are `target` + `"target_other"`. Limited-assets mismatch is `"scope_mismatch"` while the verb subject still controls `drop_initiator`.
