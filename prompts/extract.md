...
5. **Classify events.** Use the closed vocabulary in `rules/events.md` §C1. If an event doesn't fit any vocabulary item, stop — flag the ambiguity rather than invent a new label. Per `rules/events.md` §D1, when an unsolicited bid is itself the first contact, emit the `Bid` row only and note that it initiated the process; do **not** duplicate it with a standalone `Bidder Sale` row. **Per `rules/events.md` §D1.a, when that unsolicited-first-contact bidder NEVER signs an NDA in the deal** (filing narrates the target declining to engage OR the bidder withdrawing before any NDA), attach `{"code": "unsolicited_first_contact", "severity": "info", "reason": "<short summary including a ≤120-char single-quoted verbatim snippet from the filing showing the decline/withdrawal language>"}` to the Bid row to exempt it from §P-D6's NDA-before-Bid check. If the filing language is ambiguous, do NOT attach the flag — let §P-D6 fire so Austin adjudicates. **Per `rules/events.md` §D1.b, multi-activist narratives emit one `Activist Sale` row per separately-narrated activist**; collapse only when the filing describes a coordinated group. Per `rules/events.md` §K2, when a final-round invitation date and a later process-letter date differ, anchor the `... Ann` row on the earlier invitation / advancement date. **Per `rules/bids.md` §C4, concrete pre-NDA price indications emit a `Bid` row with `bid_type="informal"` plus `{"code": "pre_nda_informal_bid", "severity": "info", "reason": "<summary>"}` to document the pre-NDA timing.** §C4 does NOT need a §P-D6 exemption: the bidder's later NDA (same phase) satisfies the existence-only check.
...
7. **Handle group/joint/aggregate rows.** Follow `rules/bidders.md` §E1 (aggregate vs atomize) and §E2 (joint bidders). For unnamed parties, apply `rules/bidders.md` §E3 exactly: exact counts stay exact, `"several"` means the minimum consistent total of 3 parties, and vaguer plurals (`"multiple"`, `"various"`, `"other parties"`) emit one placeholder plus the ambiguity flag. **Two iter-4 amendments to §E2 (joint bidders):**
   - **§E2.a — Executed row exception (Class A).** An `Executed` row is **always exactly one per deal**, even when the merger-agreement counterparty is a consortium. Emit ONE `Executed` row with `bidder_alias` = the filing's merger-agreement counterparty label (e.g., `"CSC/Pamplona"`, `"Buyer Group"`) and a new `joint_bidder_members: list[str]` field listing the constituent canonical ids. The `multiple_executed_rows` hard invariant stays hard. Do **not** atomize Executed rows per-constituent.
   - **§E2.b — Group-narrated NDA aggregation (Class E).** When the filing narrates a consortium's NDA as a **single group event without per-constituent detail** (e.g., *"CSC/Pamplona executed a confidentiality agreement on 7/11/2013"*), emit ONE `NDA` row with `bidder_alias` = the consortium label, `bidder_name` = canonical id for the consortium, and a `joint_bidder_members` field. Flag: `{"code": "joint_nda_aggregated", "severity": "info", "reason": "..."}`. When the filing instead narrates each constituent's NDA separately, keep per-constituent atomization (§E2 default). Filing gives a group NDA count without per-constituent narration → emit N §E3 placeholder rows (Petsmart pattern).
...
- [ ] **Exactly one `bid_note == "Executed"` row exists.** For joint-bidder / consortium winners, still exactly one row per §E2.a, with the merger-agreement counterparty label as `bidder_alias` and `joint_bidder_members` listing constituent canonical ids.
...
- [ ] **NDA-before-bid.** Every bidder with a `Bid` row has an `NDA` row (somewhere, any order) in the same phase (§P-D6 existence check, not ordering). If a later event names a bidder who has no earlier NDA row in the current narrative, go back and emit the NDA row (even if only cited via a count-enumeration passage). **Only one exempt row type**: rows carrying `unsolicited_first_contact` (§D1.a) where the bidder never signs an NDA at all. §C4's `pre_nda_informal_bid` rows do NOT need an exemption because the bidder's later NDA satisfies §P-D6 naturally. Do NOT fabricate an NDA row to satisfy §P-D6 when the filing narrates a genuine NDA-less Bid — use the §D1.a flag instead.
- [ ] **§B3 rough-date symmetry.** `bid_date_rough` populated IFF one of `date_inferred_from_rough` / `date_inferred_from_context` / `date_range_collapsed` appears in `flags[]`. No one-sided rows (rough-without-flag OR flag-without-rough both fail §P-D2).
...
- [ ] **Joint-bidder granularity (§E2.a / §E2.b).** Executed = exactly 1 row per deal even for consortia. NDAs follow filing granularity: group-narrated NDA → ONE aggregated row with `joint_nda_aggregated` flag; individually-narrated NDAs → per-constituent rows. Bids and Drops remain per-constituent.
