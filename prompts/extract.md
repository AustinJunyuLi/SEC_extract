...
7. **Handle group/joint/aggregate rows.** Follow `rules/bidders.md` §E1 (aggregate vs atomize) and §E2 (joint bidders). For unnamed parties, apply `rules/bidders.md` §E3 exactly: exact counts stay exact, `"several"` means the minimum consistent total of 3 parties, and vaguer plurals (`"multiple"`, `"various"`, `"other parties"`) emit one placeholder plus the ambiguity flag. **Two §E2 refinements for joint bidders:**
   - **§E2.a — Executed row exception.** An `Executed` row is **always exactly one per deal**, even when the merger-agreement counterparty is a consortium. Emit ONE `Executed` row with `bidder_alias` = the filing's merger-agreement counterparty label (e.g., `"Buyer Group"`) and a new `joint_bidder_members: list[str]` field listing the constituent canonical ids. The `multiple_executed_rows` hard invariant stays hard. Do **not** atomize Executed rows per-constituent.
   - **§E2.b — Group-narrated NDA aggregation.** When the filing narrates a consortium's NDA as a **single group event without per-constituent detail** (e.g., a consortium label appearing once as the signer on a confidentiality agreement), emit ONE `NDA` row with `bidder_alias` = the consortium label, `bidder_name` = canonical id for the consortium, and a `joint_bidder_members` field. Flag: `{"code": "joint_nda_aggregated", "severity": "info", "reason": "..."}`. When the filing instead narrates each constituent's NDA separately, keep per-constituent atomization (§E2 default). Filing gives a group NDA count without per-constituent narration → emit N §E3 placeholder rows.
...
The Python validator will hard-flag structural violations (source-quote presence, vocabulary, §P-D2 rough-date symmetry, §P-D3 BidderID ordering, §P-D6 NDA-before-bid, §P-G2 bid_type trigger, §P-S4 exactly-one-Executed, etc.) and force a retry. Before returning the JSON, audit the **judgment calls** the validator can't see:

- [ ] **bid_type classification.** For every non-null `bid_type`, did you pick `informal` vs `formal` from filing language (§G1 trigger phrases) rather than gut feel? If the filing is ambiguous, did you attach a `bid_type_inference_note` per §G2 explaining the reasoning?
...
- [ ] **Bidder-identity resolution.** If a later named `Bid` row belongs to a bidder whose NDA was emitted as an unnamed §E3 placeholder, attach `unnamed_nda_promotion` on the Bid row so `pipeline.finalize()` can rewrite the placeholder. Verify `promote_to_bidder_name` is already registered in `bidder_registry`.
