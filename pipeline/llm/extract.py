...
from pipeline.deal_graph.evidence import citation_units_from_pages
...
def extractor_contract_version() -> str:
    ...
    h.update(b"\n---DEAL_GRAPH_CLAIM_SCHEMA---\n")
    h.update(schema_hash(DEAL_GRAPH_CLAIM_SCHEMA).encode("ascii"))
    return h.hexdigest()
...
def _citation_units(section_pages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return citation_units_from_pages(section_pages)
...
async def _call_prompt_only(
    *,
...
) -> tuple[dict[str, Any], CompletionResult, list[CompletionResult]]:
    ...
    return parsed, completion, [completion]
...
async def extract_deal(
    ...
    *,
    ...
    try:
        parsed, completion, completions = await _call_prompt_only(
