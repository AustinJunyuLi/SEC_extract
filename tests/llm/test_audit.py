...
def test_audit_writer_writes_prompt_call_raw_response_validation_manifest_and_latest(tmp_path):
    ...
    assert "tools_contract_version" not in manifest
    assert "repair_loop_contract_version" not in manifest
    assert "repair_turns_used" not in manifest
    assert "tool_calls_count" not in manifest
